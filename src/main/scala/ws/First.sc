...
def parseSheetRows(sheet: Sheet): Task[Option[List[Test]]] =
  ZIO.attempt {
...
def parse(excelPath: String): ZIO[Any, Throwable, TestModel] = for {
  ...
  val application: ZIO[Any,Throwable,Unit] = for {
    _ <- ZIO.logInfo("Begin application")
    testModel <- parse("/home/yakushev/docks/ufd_tests/test_suites/vers1.xlsx")
    listTest = testModel.tests.getOrElse(List.empty[Test])
    meta = testModel.meta
    _ <- ZIO.logInfo(s"META = $meta")
    _ <- ZIO.foreach(listTest){t =>
      ZIO.logInfo(s">>> TEST id = ${t.id} name = ${t.name}") *>
        ZIO.foreach(t.success_condition){sc =>
          ZIO.logInfo(s">>>>>> SUCC_COND = $sc")
        }
    }
    _ <- ZIO.logInfo("End application")
  } yield  ()

  Unsafe.unsafe { implicit u =>
    zio.Runtime.default.unsafe.run(application)
